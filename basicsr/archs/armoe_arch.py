...
class SWTForward(nn.Module):
    ...
    def __init__(self, dim, J=1, wave='db1', mode='zero'):
        super().__init__()
        ...
        # Learnable 1x1 conv to compress 4C -> C
        self.fusion = nn.Conv2d(dim * 4, dim, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        Returns:
            Tensor: Output tensor with shape :math:`(N, C_{in}, 4, H_{in}, W_{in})`
        """
        ll = x
        filts = (self.h0_col, self.h1_col, self.h0_row, self.h1_row)

        # Perform only one level of the transform (instead of a loop for multiple levels)
        out = lowlevel.afb2d_atrous(ll, filts, self.mode, 2 ** 0)  # Only one level, so 2^0

        # Apply 1x1 learnable fusion to merge 4 subbands
        out = self.fusion(out)
        return out
...
class MoE(nn.Module):
    def __init__(self, infer_mode=False, res=14, dim=96, hidden_dim=96, **kwargs):
        ...
        self.swt = SWTForward(dim=dim, J=1, wave='db1', mode='zero')
    ...
    def get_cos_map(N=224, device=torch.device("cpu"), dtype=torch.float):
        ...
        weight = torch.cos(weight_n * weight_x * math.pi) * math.sqrt(2 / N)
    ...
    def get_decay_map(resolution=(224, 224), device=torch.device("cpu"), dtype=torch.float):
        ...
        weight_n = torch.linspace(0, math.pi, resh + 1, device=device, dtype=dtype)[:resh].view(-1, 1)
        weight_m = torch.linspace(0, math.pi, resw + 1, device=device, dtype=dtype)[:resw].view(1, -1)
    ...
    def forward(self, x, H, W):
        ...
        x_dct = F.conv1d(x.contiguous().view(B, H, -1),
                         weight_cosn.contiguous().view(N, H, 1))
        x_dct = F.conv1d(x_dct.contiguous().view(-1, W, C),
                         weight_cosm.contiguous().view(M, W, 1)).contiguous().view(B, N, M, -1)

        x_fft = torch.fft.fftn(x)

        x_haar = self.haar_transform(x)

        x_swt = self.swt(x_swt).permute(0, 2, 3, 1).contiguous()
        ...
        if self.infer_mode:
            ...
            x_swt = torch.einsum("bnmc,nmc->bnmc", x_swt, self.k_exp)
        else:
            k = (torch.ones(H, W, C) * 5).to(weight_exp.device)
            weight_exp = torch.pow(weight_exp[:, :, None], self.to_k(k))
            ...
            x_swt = torch.einsum("bnmc,nmc -> bnmc", x_swt, weight_exp)

        x_dct = F.conv1d(x_dct.contiguous().view(B, N, -1), weight_cosn.t().contiguous().view(H, N, 1))
        x_dct = F.conv1d(x_dct.contiguous().view(-1, M, C),
                         weight_cosm.t().contiguous().view(W, M, 1)).contiguous().view(B, H, W, -1)

        x_fft = torch.fft.ifftn(x_fft).real

        x_haar = self.inverse_haar_transform(x_haar)

        x = torch.cat((x_dct.unsqueeze(dim=1),
                       x_fft.unsqueeze(dim=1),
                       x_haar.unsqueeze(dim=1),
                       x_swt.unsqueeze(dim=1)), dim=1)
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0.,
        ...
        self.ffn1 = SGFN(in_features=dim, hidden_features=ffn_hidden_dim, out_features=dim, act_layer=act_layer)
        self.ffn2 = SGFN(in_features=dim, hidden_features=ffn_hidden_dim, out_features=dim, act_layer=act_layer)

        # HAI
        self.gamma1 = nn.Parameter(layerscale_value * torch.ones((dim)), requires_grad=True)
        self.gamma2 = nn.Parameter(layerscale_value * torch.ones((dim)), requires_grad=True)
    ...
    def forward(self, x, x_size):
        ...
        x = x + self.ffn1(self.norm2(x), H, W)
        x = x + (res * self.gamma1)

        # part2: MoE
        shortcut = x
        x = x + self.moe(self.norm3(x), H, W)
        x = x + self.ffn2(self.norm4(x), H, W)
        x = x + (shortcut * self.gamma2)
